...
The build server has its own implementation (shared/tifile.js); this follows docs/format.md
section 4 so the two can be checked against each other.
  ...
  append_meta.py app BASE.zip -o OUT.zip [--record R] [--plan P] [--pack DIR]
      ...
      pack/<sha256> into <app>/Contents/Resources/ti/. --rename renames
...
def footer(r, p, k) -> bytes:
    s = "TIMETA1 %012d %012d %012d " % (r, p, k)
...
def strip_block(data: bytes) -> bytes:
    if len(data) >= FOOTER_LEN and data[-FOOTER_LEN:].startswith(b"TIMETA1 "):
...
def cmd_run(a):
    ...
    # after it (TI_VERIFY_BLOBS) are part of the base.
    script = base
    m = re.search(rb"^TI_VERIFY_BLOBS='([^'\n]*)'$", base, re.M)
    if m and m.group(1):
...
def cmd_app(a):
    ...
    old = apps[0][:-4]
    new = a.rename or old
    ti = "%s.app/Contents/Resources/ti/" % new
    added = {}
    if a.record:
        added[ti + "record.txt"] = open(a.record, "rb").read()
    if a.plan:
        added[ti + "plan.txt"] = open(a.plan, "rb").read()
    if a.pack:
        for sha, data in pack_files(a.pack):
            added[ti + "pack/" + sha] = data
    ...
    if a.pack and added:
        zout.writestr(zinfo(ti + "pack/", 0o40755), b"")
