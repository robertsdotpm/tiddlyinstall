...
// ticket page. What each field means is shared/form-job.js's, which the build
...
// The form, as shared/form-job.js reads it (the server reads a plain post of
...
// (web/new.js submit, below), so the wording follows the form too.
function paintWhere() {
...
// Architecture names, everywhere in this page (shared/form-job.js). 32-bit and
...
//
// The defaults themselves are unchanged (shared/form-job.js ENTRY_DEFAULTS).
...
// and shared/form-job.js's own parser decides which, so the form and the job
...
// above, and shared/form-job.js for a plain post), so both move together while
...
// The template's files, for naming the one that starts (shared/templates.js).
...
// out in shared/form-job.js (archCoverage) from the resolver's own answer, and
