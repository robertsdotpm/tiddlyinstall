...
// because three surfaces say it and they must not drift (as js/form-job.js
...
// Who says it:
//
//   js/resolve.js   the plan's `note`, which the engines print on the
//                   installer's review screen before anything is downloaded
//   js/build.js     the build page, where the publisher can still choose
...
// The plan's `note` for one target block (js/resolve.js writeTarget).
...
// The build page's line (js/build.js), the same fact told to the person
