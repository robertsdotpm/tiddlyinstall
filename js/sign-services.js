...
//
// js/authenticode.js does everything else. `beginPE` gives the digest,
...
//                     (backend/lib/signrelay.js). The credential passes
...
// panel builds (js/sign-ui.js) and the tests replace with one that points at
...
export function makeSend(opts) {
  ...
  return async function send(req) {
    ...
    if (req.relay) {
      ...
      // makes plain what it forwards and forgets: backend/lib/signrelay.js.
...
// timestamp relay follows (css/style.css .online-only).
