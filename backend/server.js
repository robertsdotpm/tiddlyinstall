...
import { runtimesSummary, packagePolicyFor, validPackage, goQuote } from '../js/resolve.js';
...
export class Server {
  ...
  async init() {
    const o = this.o;
    // resedit (vendor/, MIT) edits Windows icons in js/icon.js; the page loads
  ...
  // The site's pages and assets, not the rest of the folder.
  async siteFile(req, res, p) {
    const ok = p === '/' || p.startsWith('/css/') || p.startsWith('/js/') || p.startsWith('/img/') || p.startsWith('/vendor/') ||
