...
// SERVER is src/build_server/ (this server's own files); REPO is the
// repository root, three levels up from here.
export const SERVER = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
export const REPO = path.resolve(SERVER, '..', '..');
export const RUNTIMES = path.join(os.homedir(), 'projects', 'installer-builder-runtimes');
export const BASES = path.join(REPO, 'src', 'installers');
...
export function catalog() {
  return loadCatalog({ dir: path.join(RUNTIMES, 'catalog'), policyPath: path.join(SERVER, 'policy.json'), localRoot: '', cachePath: '/dev/null' });
}
