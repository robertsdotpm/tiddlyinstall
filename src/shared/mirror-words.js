...
// because three surfaces say it and they must not drift (as src/shared/form-job.js
...
// Who says it:
//
//   src/shared/resolve.js   the plan's `note`, which the engines print on the
//                   installer's review screen before anything is downloaded
//   src/web_client/build.js     the build page, where the publisher can still choose
...
// The plan's `note` for one target block (src/shared/resolve.js writeTarget).
...
// The build page's line (src/web_client/build.js), the same fact told to the person
