...
//
//   env.catalog        a Catalog from src/shared/resolve.js: all loaded (loadCatalogFiles),
...
//                      API through src/shared/github.js, so one form gives one
...
// Kept exported from here: src/build_server/lib/jobs.js and the tests have always
// imported it from the builder. It lives in src/shared/github.js now.
export { githubRe };
...
// (design.md 1.3; the words are src/shared/resolve.js MIRROR_GAP_WHY). A file the
...
//
// `targets` are the form's `<system>_<arch>` ids (src/shared/form-job.js
...
/* ---------- GitHub sources (src/shared/github.js) ---------- */
...
export async function runJob(r, env, progress = () => {}) {
  ...
  // those by `source github owner/repo` instead (src/build_server/server.js).
  ...
  const app = {
    ...
    // to be named after (src/shared/github.js archiveName).
...
// of src/installers/unix/ti-engine.sh tells that person what the file is and
...
// TI_VERIFY_BLOBS (src/installers/unix/make_run.sh), so one byte more or less
...
async function modeAFile(job, plat) {
  ...
  // src/build_server/test/build.test.js checks it; a nicer comment is not worth
