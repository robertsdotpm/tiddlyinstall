...
// <body>, after the data blocks). Plain ES3, like src/web_client/browser-check.js, so
...
//   #ti-js-resedit, #ti-js   the resedit bundle and the page (ES2017)
//   #ti-js-es5               both as ES5, with polyfills, raw-deflated and
//                            base64'd (tools/es5/build-es5.mjs; may be absent)
//   #ti-js-es5-inflate       src/web_client/lib/inflate.js as ES5, to unpack it
...
// copy (browser-check.js decides: window.TI_ES5_OK, IE 11 and Chrome 49)
...
(function () {
  ...
  var es5 = modern ? null : block('ti-js-es5');
  if (!modern && !(es5 && w.TI_ES5_OK === true)) return;
  ...
  w.TI_PRISTINE = '<!DOCTYPE html>\n<!-- saved from url=(0014)about:internet -->\r\n' + d.documentElement.outerHTML;
  w.TI_ES5 = !modern;
  if (modern) {
    run(block('ti-js-resedit'));
    run(block('ti-js'));
    return;
  }
  // Unpack the ES5 copy: base64, raw deflate, ASCII text.
  run(block('ti-js-es5-inflate'));
  var bin = w.atob(es5.replace(/\s+/g, ''));
  var u8 = new Uint8Array(bin.length);
  for (var i = 0; i < bin.length; i++) u8[i] = bin.charCodeAt(i);
  var out = w.__ti_inflate.inflate(u8, 'deflate-raw'), parts = [];
