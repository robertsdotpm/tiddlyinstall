...
let archCov = null;          // {windows: {...}, ...} from src/shared/form-job.js
...
function paintFiles(job) {
  ...
  $('job-files').innerHTML = files.map((f) => {
    ...
    // same table the New installer form reads (src/web_client/open-notice.js), so the
...
// Made with catalogue changes from this browser (src/web_client/local-api.js).
...
// Downloads our mirror has no copy of (design.md 1.3, src/shared/mirror-words.js).
