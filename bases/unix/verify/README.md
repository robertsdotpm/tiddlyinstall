# ibverify: the Linux and macOS bases' Ed25519 verifier
...
The code is the Windows `ibsig` plugin's: TweetNaCl 20140427's verifier
with the RFC 8032 S < L check
([../../windows/plugin-src/ed25519_verify.c](../../windows/plugin-src/ed25519_verify.c))
and its strict base64 decoder (`plancheck.c`), plus `ibverify.c` (a
...
| File | Target | SHA-256 |
| --- | --- | --- |
| `bin/ibverify-linux-x86_64` | x86_64-linux-musl | `6e2629f2f55378697bb6eac9b44c70106eb42b4958ed464f8287ec5cf570e9d4` |
| `bin/ibverify-linux-aarch64` | aarch64-linux-musl | `b3a75141dd35f4dc6f558a878f5c7769dca92267408fc3881156f8b070e50061` |
| `bin/ibverify-linux-i386` | x86-linux-musl | `ced3c7072fb5fcb58b60dcbb35fa1e87196d66763b7949b0f047bd556ac9f2db` |
| `bin/ibverify-macos-x86_64` | x86_64-macos | `1cd498383492397ee62b0e2964ea449d92a5279b3ca4f23d46ab9bc112fce6fb` |
| `bin/ibverify-macos-arm64` | aarch64-macos | `c9badcb89b6ee2e1f8314258668f960672ffa111fe27b05bf337490f72359ae2` |
...
- **Linux `.run`:** `make_run.sh` appends the three Linux binaries after
  ...
  `IB_VERIFY_BLOBS=` line with `<arch>:<offset>:<length>` for each,
...
- **macOS `.app`:** `make_app.sh` copies the two macOS binaries to
  `Contents/Resources/ibverify-x86_64` and `-arm64`, inside the bundle's
  signature.
