  ...
  function report(tag) {
    try {
      ...
      for (i = 0; i < all.length; i++) {
        if (!bar && cls(all[i], 'ib-compat-bar')) bar = all[i];
        if (cls(all[i], 'ib-page')) pages.push(all[i].getAttribute('data-page') + ':' + (all[i].offsetHeight > 0 ? 'shown' : 'hidden'));
      }
      ...
      send('state-' + tag, [
        'missing=' + h.getAttribute('data-ib-missing'), 'ready=' + h.getAttribute('data-ib-ready'), 'degraded=' + h.getAttribute('data-ib-degraded'),
        'htmlclass=' + h.className, 'errattr=' + h.getAttribute('data-ib-errors'),
        ...
        'scripts=' + d.getElementsByTagName('script').length + ' ES5copy=' + (window.ibCompat ? window.ibCompat.es5 : 'no ibCompat'),
        'errors=' + E.join(' | ')
      ].join('\n'));
