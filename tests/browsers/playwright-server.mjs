...
// serves it on ws://127.0.0.1:PORT/ib (the harness reaches that through
// ssh -L and drives it with tests/browsers/playwright.mjs) until it's
// killed. Installed on a machine as ~/ibbrowsers/playwright-<v>/server.mjs
...
const s = await webkit.launchServer({ host: '127.0.0.1', port, wsPath: '/ib', headless: true });
