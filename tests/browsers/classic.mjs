...
// src/shared/tifile.js (modes B and C carry the record the page links to; mode A is
...
import { readInstaller, recordHash } from '../../src/shared/tifile.js';
...
export async function checkFinished(t, statusUrl, { hrefs = null, name, modeLetter, out } = {}) {
  ...
  for (const f of st.files) {
    ...
    if (modeLetter === 'A') {
      ...
      t.ok(info.kind && info.record === null && !info.plan && f.name.includes(st.record), `${f.name}: src/shared/tifile.js reads it: our base, named for the record, nothing inside (mode A)`, info.kind + ' ' + f.name);
      ...
      t.ok(info.record === recText && await recordHash(info.record) === st.record, `${f.name}: src/shared/tifile.js reads it back, carrying the record`);
