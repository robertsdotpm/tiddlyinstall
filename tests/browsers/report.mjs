...
for (const name of ORDER.filter((n) => inv.machines[n])) {
  ...
  const cells = cols.map((b) => {
    ...
    return { pass: `✓ ${v}`, fail: `✗ ${v}`, incomplete: `– ${v}`,
      unsupported: `too old ${v}`, error: `error ${v}` }[kind] || kind;
  });
...
const out = [START, '',
  ...
  'with the major version it ran). ✓ every check ran and passed · ✗ a check failed · – some checks did not run, so the cell shows less than the row claims · too old: the page\'s startup check found missing features ',
