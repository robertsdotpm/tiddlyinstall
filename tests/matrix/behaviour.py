...
again on a finished install (docs/format.md section 5, `.ib-installed`).
...
def judge(p, variant, desktop_seen):
    ...
    m1 = o("marker1")
    if "ib-installed\t1" not in m1:
        bad.append("no .ib-installed after install: " + m1[:80])
    if p.get("silent2") != "0":
    ...
    m4 = o("marker4")
    if "ib-installed\t1" not in m4 or m4 == m1:
...
echo "@marker1"; cat "$A/.ib-installed" 2>/dev/null
run "$H/$F" --yes >"$H/o2" 2>&1; echo "@silent2 $?"; cat "$H/o2"; [ -e "$A/launched.txt" ] && echo launched
echo "@marker2"; cat "$A/.ib-installed" 2>/dev/null
...
def run_linux(host, variant, f):
    ...
    if host is None:
        ...
        sh(["ssh", host, "rm -rf ibbtest; mkdir -p ibbtest"], timeout=60)
        code, _, err = sh(["scp", "-q", f, f"{host}:ibbtest/{name}"], timeout=300)
        if code:
            return "fail", "scp: " + err.strip()
        code, out, err = sh(["ssh", host, env + f'SRC="$HOME/ibbtest/{name}" sh -s; rm -rf "$HOME/ibbtest"'], input=UNIX_SCRIPT)
...
echo "@marker1"; cat "$A/.ib-installed" 2>/dev/null
run --yes >"$HOME/ibbtest/o2" 2>&1; echo "@silent2 $?"; cat "$HOME/ibbtest/o2"; [ -e "$A/launched.txt" ] && echo launched
echo "@marker2"; cat "$A/.ib-installed" 2>/dev/null
IB_NO_TERMINAL=1 run --log="$HOME/ibbtest/i3.log" >"$HOME/ibbtest/o3" 2>&1; echo "@interactive $?"; tail -3 "$HOME/ibbtest/o3"
echo "@launched"; cat "$A/launched.txt" 2>/dev/null
echo "@marker3"; cat "$A/.ib-installed" 2>/dev/null
...
echo "@marker4"; cat "$A/.ib-installed" 2>/dev/null
echo "@log4"; tail -3 "$HOME/ibbtest/i4.log" 2>/dev/null
...
echo "@log"; tail -6 "$HOME/ibbtest/i1.log" 2>/dev/null
'''


def run_mac(variant, name, f):
    sh(["ssh", MAC, "rm -rf ~/ibbtest; mkdir -p ~/ibbtest"], timeout=60)
    code, _, err = sh(["scp", "-q", f, f"{MAC}:ibbtest/in.zip"], timeout=300)
    if code:
        return "fail", "scp: " + err
    code, out, err = sh(["ssh", MAC, f"VARIANT={variant} NAME={shlex.quote(name)} sh -s"], input=MAC_SCRIPT)
    sh(["ssh", MAC, "rm -rf ~/ibbtest"], timeout=60)
...
setlocal
set T=C:\ibbtest
set A=%LOCALAPPDATA%\ib\%ID%
if not defined LOCALAPPDATA set A=C:\ib\%ID%
...
if exist "C:\ib\%ID%" set A=C:\ib\%ID%
...
echo @installed
if exist "%A%\uninstall.exe" reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\ib-%ID%" >nul 2>&1 && echo uninstaller-ok
...
reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\ib-%ID%" >nul 2>&1 && echo regkey
echo @log
