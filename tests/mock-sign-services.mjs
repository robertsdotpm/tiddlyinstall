// Mock cloud signing services: one per API shape in src/web_client/sign-services.js.
//
...
// RFC 6238, independently of src/web_client/sign-services.js, so the two are not the
...
export async function startMockServices(opts) {
  ...
  // src/web_client/sign-services.js and nowhere else; this is the only place it is bent.
  const MAP = [
