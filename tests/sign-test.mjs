// Signing tests for src/web_client/lib/pkcs12.js, src/web_client/lib/authenticode.js and src/web_client/lib/pgp.js, checked
...
//        the plain-JavaScript crypto in src/web_client/lib/cryptox.js does it all)
...
import * as der from '../src/web_client/lib/der.js';
import { openPfx } from '../src/web_client/lib/pkcs12.js';
import * as ac from '../src/web_client/lib/authenticode.js';
import { parseCertBundle } from '../src/web_client/lib/x509.js';
import { readInstaller, writeInstaller, newRecordText, recordHash } from '../src/shared/tifile.js';
import * as pgp from '../src/web_client/lib/pgp.js';
import * as X from '../src/web_client/lib/cryptox.js';
import * as SS from '../src/web_client/sign-services.js';
import { checkRelay, signRelay } from '../src/build_server/lib/signrelay.js';
...
async function withRunFile() {
  const base = path.join(REPO, 'src/installers/unix/out/ti-base.run');
...
async function checkFile(name, file, caFile, certFile, { record = true } = {}) {
  ...
  if (record) {
    ...
    // src/shared/tifile.js is also the build server's reader (src/build_server/).
    ok(info.signed && info.record === RECORD, name + ': src/shared/tifile.js still reads the record');
  }
  return v;
}
...
const bases = path.join(REPO, 'src/installers/windows/out');
...
else skip('base.exe', 'src/installers/windows/out/base.exe not built');
...
/* ---------- cloud signing services (src/web_client/sign-services.js) ---------- */
//
...
await run('signing services', async () => {
  ...
  // src/build_server/lib/signrelay.js handler, over a real socket, in front of the
  ...
  try {
    ...
    /* --- the relay's allow-list (src/build_server/lib/signrelay.js) --- */
    {
    ...
    /* --- what the panel must say, before anyone types anything --- */
    {
      ...
      for (const s of SS.SERVICES) {
        ...
        if (s.where === 'server' && !/pass through the server/.test(d.credentials)) continue;
