// Unit tests for src/shared/icon.js. Open tests/icon.html over http, or headless:
...
// and src/shared/icon.js encodes PNG itself. resedit-js is loaded from the vendored
// bundle (src/vendor/resedit-bundle.js) the page includes, so no network is used.
import * as ti from '../src/shared/tifile.js';
import * as icon from '../src/shared/icon.js';
...
await run('setExeIcon on a synthetic PE', async () => {
  ok(!!globalThis.__TI_RESEDIT, 'resedit bundle is loaded');
  ...
  const info = ti.peInfo(edited);
  const dv = new DataView(edited.buffer);
  ok(dv.getUint32(info.checksumOff, true) === ti.peChecksum(edited, info.checksumOff), 'PE checksum fixed');

  // resedit re-reads the icon group and its images.
  const R = globalThis.__TI_RESEDIT;
  ...
  const opened = await ti.readInstaller(edited, 'x.exe');
  ok(!opened.signed && opened.record === null, 'edited PE opens, unsigned, no block yet');
  const RECORD = 'ti-record\t1\nname\tIcon\nproject\thello\nruntime\tpython\n';
  const withMeta = await ti.writeInstaller(opened, { record: RECORD });
  const back = await ti.readInstaller(withMeta, 'x.exe');
...
await run('macOS .app icon (icns + Info.plist)', async () => {
  ...
  const entries = [
    await ti.zipNewEntry('App.app/', null, { kind: 'dir' }),
    await ti.zipNewEntry('App.app/Contents/', null, { kind: 'dir' }),
    await ti.zipNewEntry('App.app/Contents/Info.plist', plist),
    await ti.zipNewEntry('App.app/Contents/MacOS/', null, { kind: 'dir' }),
    await ti.zipNewEntry('App.app/Contents/MacOS/install', '#!/bin/sh\n'),
    await ti.zipNewEntry('App.app/Contents/Resources/', null, { kind: 'dir' }),
  ];
  const info = await ti.readInstaller(ti.zipWrite(entries), 'App.zip');
  ...
  const newPlist = await ti.zipNewEntry(plistPath, icon.setPlistIcon(new TextDecoder().decode(await ti.zipEntryData(oldPlist)), 'AppIcon'));
  info.entries = info.entries.filter((e) => e.name !== plistPath && e.name !== icnsPath);
  info.entries.push(newPlist, await ti.zipNewEntry(icnsPath, icns));

  const built = await ti.writeInstaller(info, { record: 'ti-record\t1\nname\tApp\nruntime\tnone\n' });
  const byName = Object.fromEntries(ti.zipRead(built).map((e) => [e.name, e]));
  ok(icnsPath in byName, '.icns is in the .app Resources');
  ok(byName[icnsPath].usize === icns.length, '.icns size preserved');
  const xml = new TextDecoder().decode(await ti.zipEntryData(byName[plistPath]));
  ok(/CFBundleIconFile<\/key>\s*<string>AppIcon<\/string>/.test(xml), 'Info.plist points at AppIcon');
  const reread = await ti.readInstaller(built, 'App.zip');
  ok(reread.record.includes('name\tApp'), 'the record still reads back');
});

await run('setLinuxIcon', async () => {
  const rec = ti.parseKv('ti-record\t1\nname\tA\nruntime\tpython\n');
  const pack = [];
  ...
  ok(ti.kvGet(rec, 'icon')[0] === hash, 'record icon key names the hash');
