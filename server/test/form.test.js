...
// (docs/api.md "Plain form posts"): the body parsers, shared/form-job.js against
...
/* ---------- shared/form-job.js against new.html ---------- */
...
test('new.html: the form posts to the server, and names what the mapping reads', () => {
  ...
  // The templates' editors are rendered from shared/templates.js (web/write-editor.js):
...
test('shared/form-job.js: a plain post of the form reads as the page reads it', () => {
  ...
  // the defaults rather than asking for nothing (shared/form-job.js).
...
test('shared/form-job.js: an offline pack names its architectures, and is refused when too big', () => {
  const icon = { choice: 'default' };
...
test('new.html: the launch command is in the main form, not hidden under Customise', () => {
  ...
  // How loudly it asks depends on the source: web/new.js swaps the repo text
...
test('shared/form-job.js: a template\'s launch is used until the field is edited', () => {
  const icon = { choice: 'default' };
  ...
  // A page that put the template's own command in the field (web/new.js
...
test('shared/form-job.js: written apps take their template\'s commands (shared/templates.js)', () => {
  const icon = { choice: 'default' };
  const write = (extra) => jobFromForm(postedForm({ ...defaults(), source_kind: ['write'], mode: ['unsigned'], ...extra }), { icon });
